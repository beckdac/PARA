...
gear1_teeth = 13;
gear1_shaft_d = shaftOD + epsilon;  			// diameter of motor shaft
...
gear2_shaft_outer_d = bearing6807_2RS_d - iFitAdjust_d + bearingStepHeightInner;
		...
		union()
		{
			translate([0,0,(gear_h/2) - TT])
				gear(	twist = g1twist, 
					number_of_teeth=t1, 
					circular_pitch=cp, 
					gear_thickness = gear_shaft_h / 2 + (gear_h/2)+AT, 
			...
			//setscrew shaft
			translate([0,0,gear_h+gear_shaft_h/2-gear1_setscrew_offset])
				rotate([0,90,0])
				cylinder(r=gear1_setscrew_r, h=g1p_r);

			//setscrew captive nut
			translate([(g1p_r)/2, 0, gear_h+gear_shaft_h/2-gear1_captive_nut_r-gear1_setscrew_offset]) 
		...
		union()
		{
			...
			radial_array(n = 3) {
				translate([bearing6807_2RS_d / 3, 0, -epsilon / 2.])
					#cylinder(h = gear_h + gear_shaft_h + epsilon, d = M4ScrewDiameter + iFitAdjust);
			}
			radial_array(n = 3) {
				translate([bearing6807_2RS_d / 3, 0, -epsilon / 2.])
